...
In Lab 5, you created individual objects. In this lab, you will work with a collection of objects.

Many real programs do not manage just one object. They manage lists of objects: students in a class, items in an inventory, particles in a simulation, accounts in a bank, or records in a dataset.

You will read student score data from a CSV file, create one `StudentRecord` object for each student, store those objects in a list, and then analyze the whole class.
...
```bash
git checkout -b lab06-object-collections
```
...
```bash
git checkout main
git merge lab06-object-collections
```
